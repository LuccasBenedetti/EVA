...
### Fluxograma
graph TD
    A[Usuário Segura F2] --> B(audio.py: Gravação)
    A --> C(vision.py: Print de Tela)
    B --> D{Usuário solta F2?}
    D -- Sim --> E[transcription.py: Faster-Whisper]
    C --> F[vision.py: OCR/Contexto]
    E --> G[llm.py: vLLM + Memória]
    F --> G
    G --> H{Executar Tool?}
    H -- Sim --> I[tools.py: Automação]
    H -- Não --> J[audio.py: Edge-TTS]
    I --> J
    J --> K[Fim: Resposta de Voz]
